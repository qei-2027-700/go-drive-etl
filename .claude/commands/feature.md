---
description: Issue番号を受け取り、ブランチ作成から実装・PR・マージまでの開発フローを自動実行する。
argument-hint: <issue番号> [--manual]
# model は固定しない: 実装の主体はセッション本体なので、issue の model: ラベルに合わせて
# ユーザーが選んだセッションモデルを継承する（各ステップのスキルが必要に応じて自前で切り替える）。
---
